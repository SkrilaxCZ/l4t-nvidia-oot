...
static int jetclocks_probe(struct platform_device *pdev)
{
	...
	jetclocks_dev->cls = class_create("jetclocks");
#else
...
static inline void jetclocks_remove_wrapper(struct platform_device *pdev)
{
	jetclocks_remove(pdev);
}
#else
static inline int jetclocks_remove_wrapper(struct platform_device *pdev)
{
	return jetclocks_remove(pdev);
}
#endif
...
static struct platform_driver jetclocks_driver = {
	...
	.remove = jetclocks_remove_wrapper,
};
